...
- [x] reconcile_past_meals auto-deducts a strictly-past planned meal exactly once and is idempotent on a second call
  verify: present def reconcile_past_meals src/kroger_mcp/analytics/meal_planning.py
...
- [x] match_ingredient_to_pantry is user-scoped (accepts and forwards user_id)
  verify: present def match_ingredient_to_pantry.*user_id src/kroger_mcp/analytics/recipe_integration.py
...
- [x] classify_spice layered classifier exists and is_spice delegates to it
  verify: present def classify_spice src/kroger_mcp/analytics/ingredients.py
...
- [x] ruff is clean on src/
  verify: ruff check src/kroger_mcp/analytics/meal_planning.py src/kroger_mcp/analytics/ingredients.py
