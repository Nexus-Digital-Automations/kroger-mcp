...
- [x] Snacks are a dedicated favorites list with `list_type='snacks'` and no
  ...
  verify: present _ensure_snacks_list_for_user src/kroger_mcp/analytics/favorites.py
...
- [x] Staleness source: a new per-item `last_ordered_at`, stamped when a snack
  ...
  verify: present mark_snacks_ordered src/kroger_mcp/analytics/favorites.py
...
- [x] The check-up runs as a step inside the existing "Send to Kroger Cart"
  ...
  verify: present inSnackCheck src/kroger_mcp/web/templates/_macros/cart_send_modal.html
...
- [x] MCP `favorites(action='check_snacks')` returns the candidate checklist;
  ...
  verify: present check_snacks src/kroger_mcp/tools/favorites_tools.py
- [x] Web: `GET /api/favorites/snacks/check` returns the candidates;
  ...
  shopping list.
  verify: present snacks/check src/kroger_mcp/web/routes/api/favorites.py
- [x] When snacks need replenishing, the "Send to Kroger Cart" modal shows a
  ...
  regression).
  verify: present continueFromSnacks src/kroger_mcp/web/templates/_macros/cart_send_modal.html
- [x] Snacks actually sent to cart get `last_ordered_at` stamped (web confirm
  ...
  verify: present last_ordered_at src/kroger_mcp/analytics/favorites.py
