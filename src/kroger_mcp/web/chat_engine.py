    ...
    def chat(
        ...
        try:
            ...
            if resp.status_code != 200:
                body = resp.text[:300]
                return {'error': True, 'message': f'DeepSeek API error ({resp.status_code}): {body}'}
            return resp.json()
        except requests.Timeout:
            return {'error': True, 'message': 'DeepSeek API request timed out. Please try again.'}
        except requests.ConnectionError:
            return {'error': True, 'message': 'Could not connect to DeepSeek API. Check your internet connection.'}
...
def _handle_search_products(search_term: str, limit: int = 10) -> Dict[str, Any]:
    ...
    try:
        ...
        products = [p for p in (_extract_product(item) for item in raw) if p and p.get('product_id')]
...
def _handle_add_recipe_to_shopping_list(recipe_id: str, servings: Optional[int] = None) -> Dict[str, Any]:
...
TOOL_REGISTRY: Dict[str, Dict[str, Any]] = {
    ...
    'search_products': {
        ...
        'parameters': _p({
            'search_term': {'type': 'string', 'description': 'Product name to search for, e.g. "organic eggs"'},
    ...
    'get_low_inventory': {
        ...
        'parameters': _p({
            'threshold': {'type': 'integer', 'description': 'Level % threshold (default 20)', 'default': 20},
        }),
    },
    'list_recipes': {
    ...
    'find_deals': {
        ...
        'parameters': _p({
            ...
            'category': {'type': 'string', 'description': 'Category: dairy, meat, produce, bakery, frozen, beverages'},
        }),
    },
    'get_price_history': {
    ...
    'add_to_cart': {
        ...
        'parameters': _p({
            ...
            'modality': {'type': 'string', 'description': 'PICKUP or DELIVERY', 'default': 'PICKUP'},
            'description': {'type': 'string', 'description': 'Product description for display', 'default': ''},
        }, ['product_id']),
    },
    'remove_from_cart': {
    ...
    'add_pantry_item': {
        ...
        'parameters': _p({
            ...
            'level': {'type': 'integer', 'description': 'Inventory level 0-100%', 'default': 100},
        }, ['product_id']),
    },
    'update_pantry_level': {
    ...
    'add_recipe_to_shopping_list': {
        ...
        'parameters': _p({
            'recipe_id': {'type': 'string', 'description': 'Recipe ID'},
            'servings': {'type': 'integer', 'description': 'Number of servings (uses recipe default if omitted)'},
        }, ['recipe_id']),
    },
    'set_servings': {
...
def _generate_preview(function_name: str, args: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if function_name == 'add_to_cart':
        ...
        try:
            from kroger_mcp.tools.recipe_tools import _load_recipes
            data = _load_recipes()
            r = next((r for r in data.get('recipes', []) if r.get('id') == args.get('recipe_id')), None)
            if r:
