...
def generate_spending_report(
    ...
    try:
        ...
        top_products = sorted(
            [{"product_id": pid, **data} for pid, data in product_counts.items()],
...
def generate_patterns_report(days_back: int = 90, *, user_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not orders:
            return {"period": f"Last {days_back} days", "message": "No orders in this period"}
        ...
        by_day = [{"day": day_names[i], "orders": day_counts[i]} for i in range(7)]
        ...
        return {
            ...
            "preferred_modality": (
                max(modality_counts.items(), key=lambda x: x[1])[0] if modality_counts else None
            ),
        }
    finally:
        conn.close()
...
def generate_pantry_report(user_id: str) -> dict[str, Any]:
    ...
    try:
        ...
        if not items:
            return {"message": "No items being tracked in pantry", "total_items": 0}
    ...
    *, user_id: str,
    ...
    try:
        if include_orders:
            ...
            export["orders"] = {"count": len(orders), "data": orders}
            export["purchase_events"] = {"count": len(events), "data": events}
        ...
        if include_recipes:
            ...
            export["recipes"] = {"count": len(recipes), "data": recipes}
            export["recipe_ingredients"] = {"count": len(ingredients), "data": ingredients}
