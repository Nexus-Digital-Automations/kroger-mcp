...
def detect_category(avg_days: float | None, seasonality_score: float, total_purchases: int) -> str:
    """
...
def set_product_category(
    product_id: str, category: str, is_override: bool = True
) -> CategoryResult:
    """
    ...
    try:
        ...
        if row:
            ...
            conn.execute(
                ...
                WHERE product_id = ?
            """,
                (category, 1 if is_override else 0, datetime.now().isoformat(), product_id),
            )
        else:
            previous = None
            was_override = False
            ...
            conn.execute(
                ...
                VALUES (?, ?, ?, ?)
            """,
                (product_id, category, 1 if is_override else 0, datetime.now().isoformat()),
            )
...
def get_product_category(product_id: str) -> str | None:
    ...
    try:
        cursor = conn.execute(
            "SELECT category_type FROM products WHERE product_id = ?", (product_id,)
        )
        row = cursor.fetchone()
...
def auto_categorize_all() -> dict[str, Any]:
    ...
    try:
        ...
        return {"success": True, "categorized": changes, "total": sum(changes.values())}
    finally:
        conn.close()
