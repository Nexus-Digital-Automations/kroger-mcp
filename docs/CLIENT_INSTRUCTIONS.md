...
3. **Check safety in batch**: `safety(action='check_products', products=[{product_id, description}, ...])` — check all candidates at once
...
Prefer batch calls over looping single calls to reduce token usage and latency.

| Tool | Batch Action | Batch Parameter |
|------|-------------|-----------------|
| products | search | `search_terms: ["milk", "eggs", ...]` (max 10) |
| products | get_details | `product_ids: ["id1", "id2", ...]` (max 20) |
| safety | check_products | `products: [{product_id, description}, ...]` (max 50) |
| safety | approve_product | `products: [{product_id, description, reason}, ...]` |
| safety | block_product | `products: [{product_id, description, reason}, ...]` |
| safety | unapprove_product | `product_ids: ["id1", "id2", ...]` |
| safety | unblock_product | `product_ids: ["id1", "id2", ...]` |
| cart | add | `items: [{product_id, quantity, modality}, ...]` (max 50) |
| cart | remove | `product_ids: ["id1", "id2", ...]` |
| deals | add_to_watchlist | `product_ids: ["id1", "id2", ...]` (max 30) |
| favorites | add_item | `items: [{product_id, description, ...}, ...]` |
| favorites | remove_item | `product_ids: ["id1", "id2", ...]` |
| pantry | add / restock / remove | `product_ids: ["id1", "id2", ...]` (max 50) |
| recipes | link_ingredient | `links: [{recipe_id, ingredient_index, product_id}, ...]` |
| shopping_list | remove | `item_ids: ["id1", "id2", ...]` |
